...
def get_optimizers_and_schedulers(gen, disc):
    ...
    scheduler_discriminator = torch.optim.lr_scheduler.PolynomialLR(optim_discriminator,power=1.0,total_iters=5e5)
    scheduler_generator = torch.optim.lr_scheduler.PolynomialLR(optim_generator,power=1.0,total_iters=1e5)
    return (
        optim_discriminator,
    ...
    while iters < num_iterations:
        for train_batch in train_loader:
            with torch.cuda.amp.autocast(enabled=amp_enabled):
                ...
                gen_out = gen.forward(n_samples=train_batch.shape[0])
